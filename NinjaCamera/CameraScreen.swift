...
struct CameraScreen: View {
    ...
    private var style: CameraSkinStyle {
        selectedSkin.style
    }

    var body: some View {
        Group {
            if selectedSkin == .manuscript {
                TextFileSkinView(viewModel: viewModel, selectedSkin: $selectedSkin, style: style)
            } else {
                ZStack {
                    background
                    VStack(spacing: 0) {
    ...
    private var skinPicker: some View {
        VStack(alignment: .leading, spacing: 8) {
            Label("Skin", systemImage: "paintbrush")
            ...
            .tint(style.pickerTint)
            .colorScheme(style.pickerScheme)
        }
    }
    ...
    private var actionRow: some View {
        SlideControl(
            ...
            tint: isModeActive ? style.danger : style.accent,
            trackColor: style.sliderTrack,
            textColor: style.primaryText,
            knobTextColor: isModeActive ? style.dangerForeground : style.accentForeground,
            ...
            HStack(spacing: 10) {
                ForEach(captureModes) { mode in
                    Button(action: {
                        ...
                        .padding(.vertical, 10)
                        .background(viewModel.selectedMode == mode ? style.panelStrong : style.panelMuted)
                        .foregroundStyle(viewModel.selectedMode == mode ? style.accentForeground : style.primaryText)
    ...
    private var photoQuickAction: some View {
        Button(action: { viewModel.capturePhoto() }) {
            ...
            .padding(.vertical, 14)
            .frame(maxWidth: .infinity)
            .background(style.actionButtonBackground)
            .foregroundStyle(style.actionButtonText)
...
#Preview {
    CameraScreen(viewModel: CameraViewModel(), selectedSkin: .constant(.stealth))
}
